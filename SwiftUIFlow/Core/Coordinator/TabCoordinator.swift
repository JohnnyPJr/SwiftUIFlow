...
open class TabCoordinator<R: Route>: Coordinator<R> {
    ...
    func getTabIndex(for coordinator: any CoordinatorType) -> Int? {
...
extension TabCoordinator {
    ...
    func validateNavigationPathTabImpl(to route: any Route, from caller: (any CoordinatorType)?) -> ValidationResult {
        // First check if we can handle it directly
        if let typedRoute = route as? R, canHandle(typedRoute) {
